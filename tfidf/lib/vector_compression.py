...
def compress_vectors(vectors, n_singular_values=None):
    """
    ...
    if n_singular_values is None:
        n_singular_values = min(256, int(np.sqrt(vectors.shape[1])))

    logging.info(f'Compressing vectors from {vectors.shape[1]} to {n_singular_values} dimensions')

    u, s, vt = svds(vectors, k=n_singular_values)
    compressed_vectors = (u @ np.diag(s)).astype(np.float32)

    logging.info(f'Compressed vectors from {vectors.shape[1]} to {n_singular_values} dimensions')
